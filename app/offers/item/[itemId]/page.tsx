  ...
  return (
    <main className="min-h-screen bg-white text-ink antialiased">
      <div className="mx-auto max-w-2xl px-5 py-8 sm:px-8">
        ...
        <h1 className="mb-1 text-2xl font-semibold tracking-tight text-ink">「{item.title}」へのオファー</h1>
        <p className="mb-8 text-sm text-subtle">この商品に届いている未回答のオファー一覧です</p>
        <div className="flex flex-col gap-3">
          {offers?.map((offer) => (
            <Link key={offer.id} href={`/offers/${offer.id}`} className="flex items-center gap-4 rounded-2xl border border-hairline bg-white p-4 transition hover:border-gold hover:bg-gold-soft">
              <div className="h-14 w-14 flex-shrink-0 overflow-hidden rounded-xl bg-neutral-100">
                {offer.offering_item?.images?.[0] ? (
                  ...
                  <span className="flex h-full w-full items-center justify-center text-[10px] text-subtle">画像なし</span>
                )}
              </div>
              <div>
                <p className="text-sm font-medium text-ink">{offer.offerer?.username ?? "不明なユーザー"}さんから</p>
                <p className="text-xs text-subtle">提供品: 「{offer.offering_item?.title}」</p>
              </div>
            </Link>
          ))}
          {offers?.length === 0 && (
            <div className="rounded-2xl border border-dashed border-hairline py-16 text-center">
              <p className="text-sm text-subtle">未回答のオファーはありません。</p>
            </div>
          )}
        </div>
      </div>
