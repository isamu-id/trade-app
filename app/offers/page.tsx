...
type ItemInfo = { id: string; title: string; images: string[] | null; owner_id: string; };
type OfferRow = { id: string; status: string; offerer_id: string; offering_item: ItemInfo | null; requesting_item: ItemInfo | null; };
type CardInfo = { item: ItemInfo; tag: string; href: string; tone: "pending" | "incoming" | "done"; offerIds: string[]; };

function ItemCard({ card }: { card: CardInfo }) {
  const { item, tag, href, tone, offerIds } = card;
  const toneClass =
    tone === "pending" ? "bg-gold text-white" :
    tone === "incoming" ? "bg-ink text-white" : "bg-neutral-400 text-white";
  ...
  return (
    <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-neutral-100">
      <Link href={href} className="absolute inset-0">
        ...
        <span className={`absolute left-1.5 top-1.5 rounded-full px-2 py-0.5 text-[10px] ${toneClass}`}>{tag}</span>
...
export default async function OffersPage() {
  ...
  for (const offer of allOffers) {
    if (offer.status !== "pending" || offer.offerer_id !== myId || !offer.requesting_item) continue;
    cards.push({ item: offer.requesting_item, tag: "承諾待ち", href: `/offers/${offer.id}`, tone: "pending", offerIds: [offer.id] });
  }

  for (const offer of allOffers) {
    if (offer.status !== "pending" || offer.offerer_id === myId || !offer.requesting_item || offer.requesting_item.owner_id !== myId) continue;
  ...
  return (
    <main className="min-h-screen bg-white text-ink antialiased">
      <div className="mx-auto max-w-2xl px-5 py-8 sm:px-8">
        ...
        <h1 className="mb-8 text-2xl font-semibold tracking-tight text-ink">オファー</h1>
        {cards.length === 0 && (
          <div className="rounded-2xl border border-dashed border-hairline py-16 text-center">
            <p className="text-sm text-subtle">オファーに関する商品はありません。</p>
          </div>
        )}
